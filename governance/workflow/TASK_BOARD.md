...
| WP-0083 | Voice QC | BACKLOG | Codex | Flag weak references and degraded outputs without hard-blocking operator workflows. |
| WP-0084 | Batch dubbing | BACKLOG | Codex | Apply template/cast settings to large item sets and queue dubbing as a repeatable batch workflow. |
| WP-0085 | A/B voice previewing | BACKLOG | Codex | Generate and compare multiple clone/style variants before committing to a final voice choice. |
| WP-0086 | Export stems and alternate versions | BACKLOG | Codex | Export speech-only, background-only, final mix, and alternate dubbed variants as first-class outputs. |
| WP-0087 | Cross-episode voice memory | BACKLOG | Codex | Preserve recurring-speaker settings over time as a reusable memory layer above one-off templates. |
| WP-0088 | Reference cleanup before cloning | BACKLOG | Codex | Add optional denoise/de-reverb/isolation/normalization passes for reference clips. |
...
| WP-0090 | Subtitle-aware prosody | IN_PROGRESS | Codex | Initial line-break and punctuation shaping now flows through TTS text preparation, but per-item/template toggle and stronger hint controls remain open. |
| WP-0091 | Character libraries | BACKLOG | Codex | Add reusable fictional narrator/teaching voices independent of real recurring speakers. |
