...
| WP-0092 | Variant-aware artifact actions and logs | IN_PROGRESS | Codex | Remediation tranche: make artifact rerun/status/log handling honor variant/track/container identity for A/B previews, alternates, QC, and export packs. |
| WP-0093 | Reference cleanup integrity for multi-reference voices | IN_PROGRESS | Codex | Remediation tranche: make cleanup multi-reference aware, non-destructive by default, and collision-safe per speaker. |
| WP-0094 | Batch dubbing library-scale selection | IN_PROGRESS | Codex | Remediation tranche: remove hidden 500-item picker caps and keep batch selections stable across large libraries. |
