...
- 2026-06-04: Operator clarified that the NAS root `\\MIR\home\Video\4K Video\4K Video 21-08-2025` should remain the preferred YouTube library target and that new singles/playlists/subscriptions should live beside existing folders rather than under a `youtube` folder. Root cause for remaining single-video `/youtube` output was the persisted built-in download preset path template `{provider}/{channel}`, which bypassed the no-preset fallback fixed on 2026-05-17. Updated the built-in default and stale built-in preset normalization to `{channel}`, updated the preset editor default/placeholder, added a direct-job regression proving queued single YouTube jobs use `{channel}`, and added an existing-folder merge prompt for new saved subscriptions/playlists. Confirmed merges attach to the existing library folder and seed app-managed archive IDs from recognizable filenames without moving or deleting media. Verification passed: `cargo test --manifest-path product/engine/Cargo.toml --lib`, `npm run test:contracts`, `npm run build` in `product/desktop`, and `cargo check --manifest-path product/desktop/src-tauri/Cargo.toml`. App-boundary visual proof remains pending because no agent bridge was running.
- 2026-08-15: Closed the remaining app-boundary gate against the governed packaged v0.1.169 executable. Headless semantic and visually inspected snapshot proof now covers the nonfatal missing active-library state, Single videos / Subscriptions / Other websites tabs, per-subscription library binding, single-video search and sort direction, Options-hosted legacy 4K Video Downloader import/recovery disclosure, and explicit Firefox/Chrome/Edge/Opera YouTube browser selection. Focused desktop contracts passed 39/39 after correcting one stale assertion that still expected superseded hydration-error wording. Proof: `product/desktop/build_target/tool_artifacts/wp_runs/WP-0220/20260815-195000-v0_1_169/summary.md`.
