...
- 2026-08-15: Current v0.1.155 inspection found the remaining detector defect before closure. The Worker posts a new ping and overwrites `lastSentAt` every 100 ms even while an older ping is unanswered, so `sincePing` remains near 100 ms and cannot cross the 250 ms freeze threshold. This explains the current combination of healthy `worker_alive` rows and no historical `freeze_detected`/`freeze_recovered` rows. Selected correction: keep exactly one ping outstanding until its pong arrives, add a contract regression for that invariant, enrich freeze ingress with the instrumented backend invoke context required by acceptance, and add one bounded self-test control inside the existing Freeze events subsection. The self-test will block the WebView main thread for 750 ms and arm one controlled 750 ms heartbeat delay; the real Worker and heartbeat paths must independently emit detect/recover/skew rows. No parallel diagnostics card or temporary DevTools edit will be introduced.
- 2026-08-15: `DONE` in governed desktop v0.1.156. The packaged headless self-test produced one `freeze_detected` row (329 ms gap), one `freeze_recovered` row (671 ms total), and one `event_loop_skew` row (1,000 ms actual interval, 750 ms skew, `self_test=true`). Both freeze rows include `last_invoke=diagnostics_freeze_self_test_arm`, invoke age, completion state, and `in_flight_invoke_count=0`. The Diagnostics table rendered all three rows readably; `vvfreeze.cmd` returned 200 and printed timestamped/latest report paths. Frontend contract 19/19, production frontend build, desktop Rust check, and serialized Tauri suite 45/45 passed. Proof: `product/desktop/build_target/tool_artifacts/wp_runs/WP-0221/20260815_0330_v0_1_156/summary.md`.
