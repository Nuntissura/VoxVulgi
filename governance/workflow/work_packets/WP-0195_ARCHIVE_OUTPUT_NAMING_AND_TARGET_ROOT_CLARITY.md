...
- 2026-08-15: DONE on packaged v0.1.168. A disposable app-boundary fixture proved a managed YouTube library target and a pinned NAS override through real Tauri upserts, then the hidden semantic UI exposed both saved subscriptions with explicit `Managed under current root` / `Pinned NAS target` labels and effective paths. The Instagram one-shot surface exposed its exact default target and per-batch override before queueing. Visual snapshots were inspected, the audit returned zero unnamed controls, and focused frontend contracts passed 11/11. Proof: `product/desktop/build_target/tool_artifacts/wp_runs/WP-0195/20260815-1120_v0_1_168/summary.md`.
