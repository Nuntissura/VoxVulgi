...
- 2026-04-23: Operator smoke confirmed the remaining gap is now mostly UX and root-target clarity rather than raw archive-state loss: imported NAS-backed subscriptions still carry legacy `output_dir_override`, but newer subscriptions without overrides coexist beside VoxVulgi-managed roots in a way that is hard to distinguish or trust during normal use.
- 2026-08-15: Closed the remaining source-level clarity gap without changing storage logic or adding a card. The selected YouTube subscription detail now shows `Already-downloaded tracking` separately from `Target` and states that VoxVulgi stores continuity state separately while merging existing target-folder history when needed. Focused archiver/target/history contracts pass 13/13. Remaining before DONE: package this UI row and visually inspect it on a migrated or isolated pinned-target subscription alongside the already-recorded engine and operator smoke proof.
