...
- Windows distribution architecture (operator decision 2026-08-21): each future public full-offline release/update is one UDF ISO download named `VoxVulgi_<version>_x64_offline_full.iso`. The ISO root exposes one obvious `Install_VoxVulgi.exe` entrypoint for non-technical users. The Inno Setup 7+ entrypoint wraps the core NSIS desktop executable and extracts the complete dependency payload from external non-solid archives inside the ISO into `%APPDATA%\com.voxvulgi.voxvulgi`, including automated `pyvenv.cfg` path rewriting. Inno 7 extended-length path support is mandatory because valid bundled Python paths can exceed classic Windows `MAX_PATH`; the governed build must reject Inno 6.
