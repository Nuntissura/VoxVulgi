...
- Startup and performance diagnostics:
  ...
  - Diagnostics and overlapping Options capability checks must use one machine-readable, cost-aware demand coordinator: render cheap state first, cap Diagnostics-owned Python-heavy probes at two without changing production-job concurrency, share identical semantic flights, detach superseded waiters truthfully, and never present frontend stale-result suppression as native cancellation.
  - Startup provider verification must be one current-process full-byte flight with named phases, file/byte progress, scan count, foreground-aware checkpoints, and terminal truth. A persisted receipt alone never grants execution readiness; navigation, job start, and Diagnostics/Options probe activity may only change verifier checkpoint cadence.
  - Main-thread and Worker heartbeats must preserve source-instance plus sequence, emitted, native-received, queued, persistence-started, persistence-completed, and source-acknowledged boundaries. Shutdown must flush prior trace rows through a bounded barrier.
...
- After schema readiness, app-owned SQLite access must pass through one bounded runtime with read-only admission, serialized FIFO writer admission, explicit overload/cancellation/shutdown receipts, and bounded WAL health. Automatic Diagnostics reads never checkpoint; passive checkpoint remains an explicit operator action.
...
- Every VoxVulgi-owned `yt-dlp` process and its inherited descendants must terminate when the owning desktop process exits normally or crashes; a downloader that cannot be bound to this lifecycle must be terminated immediately and reported as a launch failure.
...
- **Localization Studio** first-screen home should use the setup-first workbench for source, output choices, run controls, progress, successful outputs, and recent-item recovery. It must not restore the retired separate `Now / Next / Last Output` orientation cards.
...
- Windows distribution architecture (operator decision 2026-08-21; measured archive layout selected 2026-08-24): each future public full-offline release/update is one UDF ISO download named `VoxVulgi_<version>_x64_offline_full.iso`. The ISO root exposes one obvious `Install_VoxVulgi.exe` entrypoint for non-technical users. The Inno Setup 7+ entrypoint wraps the core NSIS desktop executable and extracts the complete dependency payload from external bounded-solid `.7z` archives with a 64 MiB solid block inside the ISO into `%APPDATA%\com.voxvulgi.voxvulgi`, including automated `pyvenv.cfg` path rewriting. Inno 7 extended-length path support is mandatory because valid bundled Python paths can exceed classic Windows `MAX_PATH`; the governed build must reject Inno 6.
- The public ISO must not require companion `.bin` slices, manual archive extraction, terminal commands, Python/pip setup, or network downloads. Raw recursive dependency trees must not be compiled as individual Inno file entries; the governed installer uses Inno's native external archive extraction so distribution reads are sequential and the wizard remains responsive.
- The updater carries all dependencies too (operator decision 2026-08-18, delivery shape updated 2026-08-21): the same single-ISO full-offline artifact is used for an existing install and carries the complete dependency payload for its app version. An update must never require network downloads, terminal steps, or manual model/dependency repair to return the default path to a ready state.
- Running the full-offline installer over an existing install performs `Update` maintenance semantics (see 8.1.7): the user's database, preferences, and options under `%APPDATA%\com.voxvulgi.voxvulgi` are preserved, while the bundled dependency payload trees (`tools/`, `models/`, `cache/huggingface/`, `voice_backends/`) are refreshed to the versions shipped with that release. Only the explicit full actions remove user data.
- The all-dependencies contract includes the desktop shell's WebView2 runtime: the public installer must not depend on a network bootstrapper on machines without WebView2. `tauri.conf.json` must keep `bundle.windows.webviewInstallMode.type = "offlineInstaller"` so the NSIS core embeds the runtime instead of using the network bootstrapper.
- Installer diagnostics and performance are product requirements: every run must emit durable latest/timestamped logs on success, failure, or cancellation, covering named payload phase boundaries, core-installer handoff, before/after installed state, expected and observed versions/paths, verification/failure reason, and terminal outcome. The wrapper must fail instead of reporting success when the expected core registry and installed-binary versions are not proven. Representative Python-tree extraction must remain at least 2x faster than the legacy raw-file fixture. The canonical 64 MiB bounded-solid run passed at 2.128x (222.717 s raw median versus 104.677 s archive median, with identical output-tree SHA-256). A full clean-profile offline install remains a separate target of at most 30 minutes on the documented reference local-SSD machine with default security settings; the representative pass does not satisfy that gate.
...
- Busy/locked evidence may name active internal database candidates but must label all unproven ownership `external_or_unknown`; it must never infer a lock holder from elapsed time alone.
