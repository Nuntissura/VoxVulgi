...
- Desktop schema migration is startup-only. After the database-ready gate, app-owned SQLite callers use the process-shared `AppDatabase` runtime keyed by canonical database path. The frozen runtime bounds are four active readers with a 64-request admission queue and four-second admission timeout, one FIFO writer with a 64-request queue and five-second admission timeout, three classified idempotent retries, and a ten-second shutdown drain.
- Writer admission spans database work only. Filesystem, NAS, network, hashing, archive enumeration, and child-process work is prepared before admission or completed after commit. Nested writer admission fails immediately, admitted writes receive exactly one terminal receipt, and shutdown stops/joins owned job workers before database drain.
- Runtime status exposes active/waiting readers and writers, bounded recent operation receipts, transaction phase timing, WAL bytes, and passive-checkpoint outcome. A busy receipt distinguishes internal active candidates from `external_or_unknown`; SQLite cannot identify an external lock owner. Foreground Diagnostics reads never checkpoint, and the passive checkpoint command is explicit maintenance.
...
- Localization Studio owns voice-cloning runtime preflight at its entry/run surface. When the selected dub path needs a missing or repair-required managed pack, the page must show a plain-language, manifest-estimated setup/repair gate and must not queue installation until the operator explicitly chooses Setup or Repair. “Set up later” is session-scoped, keeps subtitles usable, keeps dub starts blocked, and returns after app restart; Escape performs the same defer action. The durable Phase 2 job and its resume payload remain the execution path after explicit enqueue, and successful setup re-enters the normal localization flow without a Diagnostics detour.
...
- Desktop Windows packaging uses a two-tier strategy:
  ...
  2. **Single-unit Full Offline ISO (Inno Setup 7+ orchestration)**: Produces one UDF ISO containing a root `Install_VoxVulgi.exe`, the wrapped core NSIS setup, a payload manifest, and five external bounded-solid `.7z` archives with a 64 MiB solid block for tools, models, Hugging Face cache, CosyVoice venv, and voice backends. Inno reads those archives directly from `{src}\payload` and extracts them into `%APPDATA%\com.voxvulgi.voxvulgi`; no public companion `.bin` slices or manual extraction are required.
- The single ISO is also the public update path (PRODUCT_SPEC 8.1.8): running its root installer over an existing install refreshes the dependency payload trees in `%APPDATA%\com.voxvulgi.voxvulgi` (`tools/`, `models/`, `cache/huggingface/`, `voice_backends/`) and then silently runs the core NSIS setup, whose silent-mode default maintenance action is `Update` (preferences/options and the database are preserved; only the explicit full actions remove them). Updates therefore never require network downloads for the default path.
- Inno/NSIS packaging invariants (`product/desktop/src-tauri/installer/VoxVulgi_offline_full.iss`):
  - Compile with Inno Setup 7 or newer and reject Inno 6. The bundled CosyVoice venv includes valid installed paths beyond classic Windows `MAX_PATH`; Inno 6 can extract the temporary file but fails its final `MoveFile` rename with Windows error 3, while Inno 7 supports extended-length paths throughout Setup.
  - The Inno wrapper runs with `PrivilegesRequired=lowest` so `{userappdata}` resolves to the invoking user's profile. The core NSIS setup is `perMachine` (`RequestExecutionLevel admin`), so the `[Run]` entry that launches it must keep the `shellexec` flag (ShellExecuteEx handles the UAC elevation). Without `shellexec`, Inno launches via CreateProcess, which cannot elevate and fails with Windows error 740 (`ERROR_ELEVATION_REQUIRED`) only after the multi-GB payload extraction has already completed.
  - The wrapper uses `ArchiveExtraction=enhanced/nopassword`, `DiskSpanning=no`, and `SolidCompression=no`. `SolidCompression=no` controls Inno's compiled wrapper stream only; external payload archives independently use bounded 64 MiB solid blocks. The wrapper runtime-SHA-256-verifies each mounted-ISO archive, then uses one script-level `TExtractionWizardPage.Extract` operation over five bulk archive entries into an owned AppData staging tree. It promotes only complete `tools/`, `models/`, `cache/huggingface/`, and `voice_backends/` roots by same-volume directory rename with a recoverable backup; `[Files] extractarchive` is forbidden because Inno processes it member-by-member and regresses Python-tree installation speed. Raw recursive payload `Source` entries and public `setup-*.bin` slices are also forbidden.
  - Payload archives use fast LZMA2 with a 64 MiB solid block. The governed build tests every archive, rejects absolute/up-level/link entries, and records source file count/bytes, archive bytes/SHA-256, destination root, solid-block policy, and 7-Zip version before ISO creation.
  - The ISO is UDF 1.02 created by Microsoft Oscdimg with one obvious root entrypoint. The governed build lists the completed ISO and verifies the exact root installer, manifest, README, and archive paths before publication.
  - `SetupLogging=yes`; every run continuously checkpoints a latest log and retains a timestamped final log in `%APPDATA%\com.voxvulgi.voxvulgi\diagnostics\installer` on success, failure, or cancellation. Structured event rows cover every named payload phase boundary, the core handoff, before/after installed state, expected and observed registry/install-path/binary versions, verification or failure reason, and terminal outcome.
  - The core `[Run]` entry keeps `shellexec waituntilterminated`; Inno's incompatible `logoutput` flag is not used. After the elevated process returns, the wrapper independently requires the expected uninstall-registry version and an installed main binary with a matching file version. Any mismatch raises setup failure and preserves the detailed log, preventing a stale installed version from being presented as success.
  - Performance gates compare a representative Python-tree fixture against the legacy raw-file layout (at least 2x faster) and time the complete clean-profile/offline install on a documented local-SSD reference machine (target at most 30 minutes). The selected 64 MiB bounded-solid fixture passed at 2.128x (222.717 s raw median versus 104.677 s archive median, with identical output-tree SHA-256). Fixture proof cannot replace full-payload proof or the 30-minute gate.
...
- Provider dependency authentication is one current-process semantic flight over the pinned complete provider tree. The complete-tree proof includes `server/node_modules`, so the pinned child digest is published without a duplicate second walk. Progress is revisioned by named phase, files/bytes, scan count, committed source identity, and terminal state; same-process explicit verification re-authenticates bytes, while concurrent startup/catalogue/job-readiness consumers share the active flight.
- The verifier uses bounded file checkpoints and short expiring foreground-demand leases from page navigation, job start, and Diagnostics/Options probes. These leases change only verification yield cadence; production job admission/concurrency is unchanged. Persisted integrity receipts remain audit history and never authorize provider execution without the current process attestation.
...
- Unified library and duplicate prevention:
  ...
  - On Windows, all VoxVulgi-owned `yt-dlp` launches are assigned to one process-local Job Object configured with `JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE`. The desktop exit hook explicitly terminates the Job Object; Windows last-handle cleanup covers abrupt termination/crashes and inherited downloader descendants. Assignment is fail-closed: an unbound child is killed before the launch returns an error.
...
- Diagnostics/Options capability demand is coordinated app-wide by semantic operation key, cost class, ownership generation, freshness identity, and a unique flight span. The coordinator admits at most four Diagnostics operations and at most two Diagnostics-owned Python-heavy probes, shares duplicate Torch/Demucs/catalog work, removes queued superseded work, and records waiter detachment separately from backend-acknowledged cancellation. Production inference/job concurrency is outside this limit.
- Heavy capability probes return explicit `ready`, missing, failed, timeout, stale, or superseded truth. Cache invalidation increments an engine epoch so an invalidated in-flight result cannot republish stale readiness. The YouTube protection pane consumes one bounded deferred read transaction containing both operation lanes and bounded history rather than replaying history on mount.
- Heartbeat trace ingress keys sequence truth by source plus source-instance. Persistence completion is timestamped only after trace locking/rotation/append returns; source acknowledgement includes that completion receipt. A FIFO diagnostics flush barrier is mandatory at orderly shutdown after owned worker join and database drain receipts.
