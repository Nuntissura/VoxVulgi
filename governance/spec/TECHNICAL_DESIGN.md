...
- Inno/NSIS packaging invariants (`product/desktop/src-tauri/installer/VoxVulgi_offline_full.iss`):
  ...
  - The wrapper uses `ArchiveExtraction=enhanced/nopassword`, `DiskSpanning=no`, and `SolidCompression=no`. Each payload `[Files]` entry uses `external extractarchive recursesubdirs createallsubdirs ignoreversion` and an `ExternalSize` generated from the archive manifest. Raw recursive payload `Source` entries and public `setup-*.bin` slices are forbidden.
  - Payload archives use fast LZMA2 with solid mode disabled. The governed build tests every archive, rejects absolute/up-level/link entries, and records source file count/bytes, archive bytes/SHA-256, destination root, and 7-Zip version before ISO creation.
  ...
  - Performance gates compare a representative Python-tree fixture against the legacy raw-file layout (at least 2x faster) and time the complete clean-profile/offline install on a documented local-SSD reference machine (target at most 30 minutes). Fixture proof cannot replace full-payload proof.
