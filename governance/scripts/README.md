...
- `build_desktop_target.ps1` - builds the desktop app with `CARGO_TARGET_DIR` pinned to `product/desktop/build_target/Current`, prepares the offline bundle payload (Phase 1 + Phase 2) for governed release packaging, archives prior build outputs to `product/desktop/build_target/old_versions`, writes run logs to `product/desktop/build_target/logs`, auto-bumps desktop semantic version, and appends a `governance/release/BUILD_CHANGELOG.md` entry with included Work Packet IDs (`-WorkPackets` or `VOXVULGI_BUILD_WP_IDS` required). Routine builds should follow `build_rules.md` and reuse a verified payload when bundled dependency inputs did not change.
