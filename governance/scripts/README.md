...
- `build_desktop_target.ps1` - builds the desktop app with `CARGO_TARGET_DIR` pinned to `product/desktop/build_target/Current`, validates/reuses the existing offline bundle payload by default, refreshes it only when missing/stale or when `-RefreshOfflinePayload` / `-ForceRefreshOfflinePayload` is passed, archives prior build outputs to `product/desktop/build_target/old_versions`, writes run logs to `product/desktop/build_target/logs`, auto-bumps desktop semantic version, and appends a `governance/release/BUILD_CHANGELOG.md` entry with included Work Packet IDs (`-WorkPackets` or `VOXVULGI_BUILD_WP_IDS` required). Use `-ValidateOfflinePayloadOnly` to check the current payload without bumping/building.
