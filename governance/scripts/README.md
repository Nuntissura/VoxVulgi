...
- `build_desktop_target.ps1` - builds the desktop app with `CARGO_TARGET_DIR` pinned to `product/desktop/Build Target/Current`, refreshes the offline bundle payload (Phase 1 + Phase 2) before packaging, archives prior build outputs to `product/desktop/Build Target/Old versions`, writes run logs to `product/desktop/Build Target/logs`, auto-bumps desktop semantic version, and appends a `governance/release/BUILD_CHANGELOG.md` entry with included Work Packet IDs (`-WorkPackets` or `VOXVULGI_BUILD_WP_IDS` required).
