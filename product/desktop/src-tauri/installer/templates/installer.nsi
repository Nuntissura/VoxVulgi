...
Function PageMaintenanceModeInfo
  ...
  wix_intro_loop:
    ...
    StrCmp $1 "" wix_intro_loop_done ; Exit loop if there is no more keys to loop on
    IntOp $0 $0 + 1
    ReadRegStr $R0 HKLM "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\$1" "DisplayName"
    ReadRegStr $R1 HKLM "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\$1" "Publisher"
    StrCmp "$R0$R1" "${PRODUCTNAME}${MANUFACTURER}" 0 wix_intro_loop
    ReadRegStr $R0 HKLM "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\$1" "UninstallString"
    ${StrCase} $R1 $R0 "L"
    ${StrLoc} $R0 $R1 "msiexec" ">"
    StrCmp $R0 0 0 show_intro_page
  wix_intro_loop_done:

  ; Check if there is an existing NSIS installation, if not, skip intro page.
  ...
  nsDialogs::Show
FunctionEnd
Function PageLeaveMaintenanceModeInfo
FunctionEnd
